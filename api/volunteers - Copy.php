...
if ($fn === 'add') {
    ...
    $village = intval($_POST['village_id'] ?? 0) ?: NULL;//clean_string($_POST['village'] ?? '');
    ...
    $vill = ($village_id and $village_id > 0) ? $village_id : NULL;
    $occ = ($occupation_id and $occupation_id > 0) ? $occupation_id : NULL;
    $seva = ($seva_interest_id and $seva_interest_id > 0) ? $seva_interest_id : NULL;

    $stmt = $conn->prepare("INSERT INTO volunteers (name, village, city, state, country, mobile, email, phone, occupation_id, seva_interest_id, dob, remarks) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssisss", $name, $village, $city, $state, $country, $mobile, $email, $phone, $occ, $seva, $dob, $remarks);
    if (!$stmt->execute()) {
    ...
    $res = $conn->query("
        SELECT v.id, v.name, v.mobile, v.email, v.village, v.city, v.state, v.country,
               v.phone, v.dob, v.remarks,
               o.occupation_name, s.seva_name, DATE_FORMAT(v.created_at, '%Y-%m-%d %H:%i') AS created_at
        FROM volunteers v
